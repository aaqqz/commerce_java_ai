...
- 프로젝트 가이드라인: 4계층, Facade 사용, Logic Layer의 Repository 접근 허용, @Transactional 규칙 등을 준수한다.
...
- 스키마/엔티티/개념 객체 확장
  - 현재 `CouponEntity`/`Coupon`는 `BigDecimal discount` 1개만 존재.
  ...
  - 추가 필드 제안(옵션)
    - `BigDecimal maxDiscountAmount` 정률 할인 상한액 (nullable)
    - `BigDecimal minOrderAmount` 최소 주문 금액 조건 (nullable)
- Data Access Layer
...
- Facade 사용: Controller는 Service만 조합. Logic/Repository 직접 참조 금지.
